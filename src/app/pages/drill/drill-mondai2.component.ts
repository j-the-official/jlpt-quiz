...
import { Mondai2Question } from '../../models/question.model';
  ...
  template: `
    <div class="mx-auto max-w-2xl px-4 py-8">
      ...
      <div class="mb-2 flex items-center gap-2">
        <span class="inline-flex items-center rounded-md bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary">問題 2</span>
        <span class="text-sm text-muted-foreground">文意推斷（文脈規定）</span>
      </div>
...
export class DrillMondai2Component {
  private questionService = inject(QuestionService);
  private progressService = inject(ProgressService);

  questions = signal<Mondai2Question[]>([]);
  ...
  constructor() {
    this.questionService.getQuestions(QuestionType.MONDAI_2, { shuffle: true }).subscribe(qs => {
      this.questions.set(qs as Mondai2Question[]);
    });
  }

  getChoiceState(index: number): ChoiceState {
    if (!this.submitted()) {
      return this.selectedIndex() === index ? 'selected' : 'default';
    }
    const q = this.currentQuestion();
    if (!q) return 'default';
  ...
  selectChoice(index: number): void {
    if (!this.submitted()) this.selectedIndex.set(index);
  }
  ...
  submit(): void {
    ...
    const q = this.currentQuestion();
    if (q) {
      this.progressService.recordAttempt(q.id, QuestionType.MONDAI_2, this.selectedIndex(), this.isCorrect());
    }
  }
